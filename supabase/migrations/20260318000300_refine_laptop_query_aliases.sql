...
AS $$
  ...
  expanded_query_tokens AS (
    SELECT token
    FROM base_query_tokens
    UNION
    SELECT unnest(
      CASE token
        WHEN 'laptop' THEN ARRAY['macbook', 'surface', 'notebook', 'ultrabook']
        WHEN 'computer' THEN ARRAY['laptop', 'macbook', 'surface', 'desktop', 'pc']
        WHEN 'phone' THEN ARRAY['smartphone', 'iphone', 'android', 'mobile']
        WHEN 'car' THEN ARRAY['vehicle', 'bmw', 'toyota', 'honda', 'sedan', 'suv']
        WHEN 'shampoo' THEN ARRAY['hair', 'haircare', 'beauty', 'conditioner']
        ELSE ARRAY[]::text[]
      END
    )
    FROM base_query_tokens
  ...
  scored AS (
    SELECT
      ...
      (
        ...
        FROM query_tokens qt
        WHERE
          coalesce(lower(p.metadata ->> 'subCategory'), '') ILIKE '%' || qt.token || '%'
          OR coalesce(lower(p.metadata ->> 'mainCategory'), '') ILIKE '%' || qt.token || '%'
          OR coalesce(lower(c.name_key), '') ILIKE '%' || qt.token || '%'
      ) AS category_overlap_count
  ...
  ranked AS (
    SELECT
      ...
      CASE
        WHEN token_stats.token_count > 0
          THEN scored.category_overlap_count / token_stats.token_count
        ELSE 0
      END AS category_overlap_ratio,
      CASE
  ...
  SELECT
    ...
    (
      ranked.semantic_score * 0.5
      + ranked.keyword_score * 0.1
      + ranked.token_overlap_ratio * 0.18
      + ranked.category_overlap_ratio * 0.17
  ...
  WHERE
    (
      ...
      OR ranked.category_overlap_ratio >= 0.5
    )
    AND (
      ranked.token_overlap_ratio > 0
      OR ranked.category_overlap_ratio > 0
